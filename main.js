...
function initPuzzle() {
  ...
  container.innerHTML = "";
  const img = "assets/img/gato1.jpg";
  const pieces = [];
...
document.getElementById("startPuzzleBtn").addEventListener("click", initPuzzle);

/* 2.1 Blog */
const blogPosts = [
  { title: "Cómo cuidar a un gato", text: "Consejos básicos para mantener a tu gato sano y feliz." },
  { title: "Por qué los gatos amasan", text: "Un comportamiento adorable con explicación científica." },
  { title: "Cómo elegir la mejor comida", text: "Guía rápida para entender los tipos de alimento." }
];
...
function initTest() {
  ...
  container.innerHTML = `
    <p>¿Qué tipo de gato prefieres?</p>
    <button class="secondary-button" data-type="tranquilo">Tranquilo</button>
    <button class="secondary-button" data-type="jugueton">Juguetón</button>
    <button class="secondary-button" data-type="curioso">Curioso</button>
  `;
  container.querySelectorAll("button").forEach(btn => {
    btn.addEventListener("click", () => {
      container.innerHTML = "<p>Tu gato ideal es: <strong>" + btn.dataset.type + "</strong></p>";
    });
  });
}
initTest();
...
function renderTimeline() {
  ...
  timelineData.forEach(item => {
    ...
    div.innerHTML = `<strong>${item.year}</strong>: ${item.text}`;
    container.appendChild(div);
  });
}
renderTimeline();
...
function render3DCards() {
  ...
  cats.slice(0, 6).forEach(cat => {
    ...
    div.innerHTML = `<h3>${cat.nombre}</h3><p>${cat.raza}</p>`;
    container.appendChild(div);
  });
}
render3DCards();

/* 5.1 Generador de nombres */
const names = ["Michi", "Pelusa", "Neko", "Bigotes", "Copo", "Sombra", "Kuro", "Luna"];
...
/* 5.2 Calculadora de edad felina */
document.getElementById("calcAgeBtn").addEventListener("click", () => {
  const years = parseInt(document.getElementById("humanYearsInput").value);
  if (!years) return;
  const catYears = years === 1 ? 15 : years === 2 ? 24 : 24 + (years - 2) * 4;
  document.getElementById("catAgeResult").textContent = `Edad felina: ${catYears} años`;
});

/* 5.3 Recomendador */
document.getElementById("recommendBtn").addEventListener("click", () => {
  const type = document.getElementById("personalitySelect").value;
  const result = document.getElementById("recommendationResult");

  const map = {
    tranquilo: "Cama suave + rascador bajo.",
    jugueton: "Pelotas, cañas y juguetes interactivos.",
    curioso: "Torres altas y túneles.",
    independiente: "Camas elevadas y juguetes automáticos."
  };

  result.textContent = map[type] || "Selecciona una opción.";
});
